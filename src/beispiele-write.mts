...
const neuesFahrzeug: Prisma.GebrauchtwagenCreateInput = {
    fin: 'WVWZZZ1JZXW999999',
    marke: 'VW',
    modell: 'Polo',
    baujahr: 2022,
    erstzulassung: '2022-06-01T00:00:00Z',
    kilometerstand: 18000,
    kraftstoffart: 'BENZIN',
    fahrzeugklasse: 'KLEINWAGEN',
    ausstattung: { navi: true, sitzheizung: false },
    schadenfrei: true,
    anbieterUsername: 'haendler-test',
    kontaktEmail: 'test@example.invalid',
    standort: {
        create: {
            plz: '76131',
            ort: 'Karlsruhe',
...
type FahrzeugCreated = Prisma.GebrauchtwagenGetPayload<{
    include: {
        standort: true;
        schaeden: true;
    };
...
const geaendertesFahrzeug: Prisma.GebrauchtwagenUpdateInput = {
    version: { increment: 1 },
    kilometerstand: 20000,
    schadenfrei: false,
    anbieterUsername: 'haendler-test-updated',
};
type FahrzeugUpdated = Prisma.GebrauchtwagenGetPayload<{}>; // eslint-disable-line @typescript-eslint/no-empty-object-type

// Schreib-Operationen mit dem Model "Gebrauchtwagen"
try {
    await prisma.$connect();
    await prisma.$transaction(async (tx) => {
        // Neuer Datensatz mit generierter ID
        const fahrzeugDb: FahrzeugCreated = await tx.gebrauchtwagen.create({
            data: neuesFahrzeug,
            include: { standort: true, schaeden: true },
        });
        message = styleText(['black', 'bgWhite'], 'Generierte ID:');
        console.log(`${message} ${fahrzeugDb.id}`);
        console.log();

        // Version +1 wegen "Optimistic Locking"
        const fahrzeugUpdated: FahrzeugUpdated = await tx.gebrauchtwagen.update({
            data: geaendertesFahrzeug,
            where: { id: 1 },
        });
        // eslint-disable-next-line require-atomic-updates
        message = styleText(['black', 'bgWhite'], 'Aktualisierte Version:');
        console.log(`${message} ${fahrzeugUpdated.version}`);
        console.log();

        const geloescht = await tx.gebrauchtwagen.delete({ where: { id: fahrzeugDb.id } });
        // eslint-disable-next-line require-atomic-updates
